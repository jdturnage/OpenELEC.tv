...
def phoneHome():
    ...
    serverIP = __addon__.getSetting("server")
    ...
    try: 
        ...
        data = response.read()
	xbmc.log(data)
	xbmc.log(simplejson.loads(data)['piDee'])
	__addon__.setSetting("PiDee",str(simplejson.loads(data)['piDee']))
...
def wait_for_internet():
    ...
    while True:
	try:
	    response = urllib2.urlopen('http://www.nasa.gov',timeout=10)
 	    break
...
if (__name__ == "__main__"):
    ...
    if(call(["/usr/bin/nasawifi",server]) != 0):
